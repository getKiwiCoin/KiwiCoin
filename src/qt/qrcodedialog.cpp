...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("KIWI")
    updateDisplayUnit();
}
...
{
    QString ret = QString("kiwicoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non KIWI unit input in lnReqAmount, we generate the URI with KIWI as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::KIWI, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
